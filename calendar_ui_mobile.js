...
function rcube_calendar_ui(settings)
{
    ...
    var event_show_dialog = function(event)
    {
      ...
      // MANTIS 3609: Le bandeau privé n'est pas retiré une fois qu'on ouvre un évènement privé
      else {
      ...
      $("#event_show_edit_button").unbind("click");
	  $('#event_show_edit_button').click(function() {
		  event_edit_dialog('edit', event);
	  });
	  $("#event_show_remove_button").unbind("click");
	  $('#event_show_remove_button').click(function() {
		  if (me.delete_event(event)) {
			$.mobile.pageContainer.pagecontainer("change", "./?_task=calendar");  
		  }		  
	  });
	  $("#event_show_back_button").unbind("click");
	  $('#event_show_back_button').click(function() {
		  $.mobile.pageContainer.pagecontainer("change", "./?_task=calendar");
	  });
	  $.mobile.pageContainer.pagecontainer("change", $("#eventshowpage"));
...
*/
    ...
    {
      // PAMELA
      var $dialog = $('<div id="eventeditdialog">');
      ...
      $("#event_edit_save_button").unbind("click");
	  $('#event_edit_save_button').click(function() {
        ...
        if ((data.attendees.length || (event.id && /* PAMELA */event.attendees && event.attendees.length)) && allow_invitations && (notify.checked || invite.checked)) {
          data._notify = 1;
        }
	  ...
	  $("#event_edit_back_button").unbind("click");
	  $('#event_edit_back_button').click(function() {
		  $.mobile.pageContainer.pagecontainer("change", "./?_task=calendar");
	  });

      if (event.id) {
    	// MANTIS 3634: Ajouter un bouton enregistrer comme nouveau
    	  if (!event.recurrence) {
    		$("#event_edit_remove_button").show();
    		$("#event_edit_remove_button").unbind("click");    		
    		$('#event_edit_remove_button').click(function() {
    			if (me.delete_event(event)) {
    				$.mobile.pageContainer.pagecontainer("change", "./?_task=calendar");
    			}
    		});
    		$("#event_edit_saveasnew_button").show();
    		$("#event_edit_saveasnew_button").unbind("click");
    		$('#event_edit_saveasnew_button').click(function() {
    			var start = parse_datetime(allday.checked ? '12:00' : starttime.val(), startdate.val());
	            var end   = parse_datetime(allday.checked ? '13:00' : endtime.val(), enddate.val());
      ...
      $.mobile.pageContainer.pagecontainer("change", $("#eventeditpage"));
    ...
    this.calendar_edit_dialog = function(calendar)
    {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          color = $('#calendar-color').val(calendar.editcolor).miniColors({ value: calendar.editcolor, colorValues:rcmail.env.mscolors });
    ...
    // PAMELA - Refresh skin mobile
    this.refresh_calendar = function() {
    	var view = fc.fullCalendar('getView');
    	if (view.name == 'agendaDay') {
    		$('.jqm-calendar-header h1').text($('#calendar .fc-view-agendaDay thead .fc-widget-header').text());
    		$('.jqm-calendar #calendar .fc-view-agendaDay thead').hide();
    	} else {
    		$('.jqm-calendar-header h1').text(view.title.replace('&mdash;', '-'));
    	}
    };
    ...
    for (var id in rcmail.env.calendars) {
      ...
      this.calendars[id].editcolor = '#' + cal.color;
    ...
    // PAMELA - Pas de header en skin mobile
    var header, defaultView, allDayText;
    if (rcmail.env.ismobile) {
    	header = false;
    	defaultView = rcmail.env.view || settings['default_view'];
    	if (defaultView == 'agendaWeek' && !$('#agendaweekbutton').is(':visible'))
    		defaultView = 'agendaDay';
    	allDayText = '';
    } else {
    	header = {
    ...
    var fc = $('#calendar').fullCalendar({
      ...
      eventDrop: function(event, dayDelta, minuteDelta, allDay, revertFunc) {
        if (event.end == null || event.end.getTime() < event.start.getTime()) {
        	// MANTIS 3567: Problème lorsque l'on move un evenement journée entière
        	//	event.end = new Date(event.start.getTime() + (allDay ? DAY_MS : HOUR_MS));
        	event.end = new Date(event.start.getTime() + (allDay ? 0 : HOUR_MS));
        }
...
//          event.start.setHours(12);
    	  if (event.allDay && event.end)
    		  event.end = new Date(event.end.getTime() + DAY_MS);
    	  else if (event.allDay)
    		  event.end = new Date(event.start.getTime() + DAY_MS);
    ...
    var init_calendar_ui = function()
    {
      ...
      if (!rcmail.env.ismobile) {
    	  ...
    	  $('#edit-startdate').change(function(){ shift_enddate(this.value); });
    	  $('#edit-enddate').change(event_times_changed);
      } 
