    ...
    var event_edit_dialog = function(action, event)
    {
      ...
      if (event.id) {
        ...
        if (!event.recurrence) {
          // save action
          buttons.push({
            text: rcmail.gettext('saveasnew', 'calendar'),
            'class': 'mainaction',
            click: function() {
            var start = parse_datetime(allday.checked ? '12:00' : starttime.val(), startdate.val());
            var end   = parse_datetime(allday.checked ? '13:00' : endtime.val(), enddate.val());
...
// PAMELA - Rendre cal accessible ailleurs que dans ce fichier
var cal;
...
window.rcmail && rcmail.addEventListener('init', function(evt) {
  ...
  // let's go
  window.cal = new rcube_calendar_ui($.extend(rcmail.env.calendar_settings, rcmail.env.libcal_settings));
