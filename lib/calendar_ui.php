...
class calendar_ui
{
  ...
  public function init()
  {
    ...
    // PAMELA - Modification du calendrier pour charger la version mobile
    ...
    // PAMELA - Chargement de la skin mobile
    if ($this->rc->config->get('ismobile', false)) {
  ...
  public function addJS()
  {
    // PAMELA - Modification du calendrier pour charger la version mobile
    if ($this->rc->config->get('ismobile', false)) {
      $this->cal->include_script('calendar_ui_mobile.js');
    }
    else {
  ...
  function attendees_form($attrib = array())
  {
    ...
    $a = html::a(array('href' => '#', 'class' => 'search_anaisweb_attendees anaisweb_amande_button anaisweb_amande_icon', 'title' => $this->rc->gettext('search_title', 'melanie2_anaisweb')), '');
