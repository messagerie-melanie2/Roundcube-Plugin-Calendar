...
class melanie2_driver extends calendar_driver {
  ...
  public function list_calendars($filter = 0, $tree = null, $calid = null) {
    ...
    if (melanie2_logs::is(melanie2_logs::DEBUG))
      melanie2_logs::get_instance()->log(melanie2_logs::DEBUG, "[calendar] melanie2_driver::list_calendars(filter = $filter)");
    ...
    try {
      ...
      if (isset($prop['id'])) {
        ...
        if (isset($this->calendars[$id])) {
          ...
          if (isset($prop['name']) && $cal->owner == $this->user->uid && $prop['name'] != "" && $prop['name'] != $cal->name) {
            $cal->name = $prop['name'];
            $cal->save();
          }
          // Récupération des préférences de l'utilisateur
  ...
  private function _write_postprocess(LibMelanie\Api\Melanie2\Event $_event, $event, $new, $move = false) {
    ...
    if (isset($event['start'])) {
      if (isset($event['allday']) && $event['allday'] == 1) {
        ...
        if ($event['start']->getTimezone()->getName() != $timezone->getName()) {
    ...
    if (isset($event['end'])) {
      if (isset($event['allday']) && $event['allday'] == 1) {
        ...
        if ($event['end']->getTimezone()->getName() != $timezone->getName()) {
  ...
  private function _read_postprocess(LibMelanie\Api\Melanie2\Event $event, $freebusy = false, $isexception = false) {
    ...
    if (substr($event->start, - strlen('00:00:00')) === '00:00:00' && substr($event->end, - strlen('00:00:00')) === '00:00:00') {
      $_event['allday'] = 1;
      ...
      $timezone = new \DateTimeZone($this->calendars[$event->calendar]->getTimezone());
      if ($this->cal->timezone->getName() != $timezone->getName()) {
        $_event['start']->setTimezone($timezone);
        $_event['end']->setTimezone($timezone);
      }
    }
    ...
    if ($freebusy) {
      ...
      if (isset($event->status)) {
        $_event['free_busy'] = melanie2_mapping::m2_to_rc_free_busy($event->status);
        $_event['status'] = melanie2_mapping::m2_to_rc_status($event->status);
      }
